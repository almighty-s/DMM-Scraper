...
def performer_by_url(url):
    ...
    if aliases_xpath_result := get_xpath_result(tree, XPATHS["aliases"]):
        if not isinstance(aliases_xpath_result, list):
            aliases_xpath_result = [aliases_xpath_result]
    ...
    if measurements_result := get_xpath_result(tree, XPATHS["measurements"]):
        ...
        if match := re.search(REGEXES["measurements"], convert_to_halfwidth(combined)):
            if lang == "JP":
                ...
                # Clean cup string for bra conversion (remove 'カップ' etc)
                cup_cleaned = match["cup"].replace("カップ", "")
                bra_size = convert_bra_jp_to_us(f'{match["bust"]}{cup_cleaned}')
...
def performer_by_name(name: str, retry=True) -> None:
    ...
    if re.search(REGEXES["url"], result.url):
        ...
        for node in search_result:
            ...
            if not node_value or "/" not in node_value:
                continue
...
if __name__ == "__main__":
    try:
        ...
        try:
            ...
            pass
        print(json.dumps({"error": str(e), "traceback": err_msg}))
